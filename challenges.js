...
// ++ infront of var is changed then evaluated.
// if ++ is after the ++ then it's evaluated first then changes the value.
...
const addTwoNumbers = (numO, numT) => {
  // if (Number.isInteger(numO) && Number.isInteger(numT) ) {
  if ((typeof numO === 'number') && (typeof numT === 'number')){
    return numO + numT
  }
  else
    return NaN 
}
//Learned to return NaN and not 'NaN'
//Learned of Number.isInterger //I couldn't do the other way from member thought I recalled it, so I did it also.
...
//Learned how to check if an array is empty.
//I did see online that Array.isArray() will check if something is an array.
...
function addList() {
  ...
  // newArray.reduce((sum, number) => {
  //   return sum + number
  // })
}
//Wasted a LOT of time learning I can't do this with the => function methied.
/*-----------------------------------------------------------------
...
const computeRemainder = (num, divider) => {
  if (divider === 0) return Infinity
  //return num % divider
  //for fun
  return num - (divider * Math.floor(num / divider))
}
//Got it in one. Took more work then I like to admit on the fun part. Might need to try things out by hand next time.
...
// Hope to have learned the mental shortcut for that the i < length is one less then what you would think, is only for .length.
...
// Hammered in that split turns string into array. "" to split every part.
// .reverse() is only for Arrays. Not strings
// .join is to put an array in a single string. You pass what you want the string to have between the "arrays" 
/*-----------------------------------------------------------------
...
const removeEnds = str => {
  // if (str.length <= 3)
  // return str.split().pop().splice(0)
  return str.slice('1', -1)
}
//Hopefully I'll do this enough I'll not need to keep looking up slice and things like it.
/*-----------------------------------------------------------------
...
const formatWithPadding = (number, pad, padAmount) => {
  let numberArr = number.toString().split('')
  while (numberArr.length < padAmount) {
    numberArr.unshift(pad)
  }
...
const hammingDistance = (one, two) => {
  let count = 0
  if (one.length !== two.length) return NaN
...
///////////////
const mumble = str => {
  return str.split('').reduce((results, char, i) => (
     results += ((i||'')&&'-') + char.repeat(i+1)
  ), '')
}
/*-----------------------------------------------------------------
...
const fromPairs = (arrOb) => {
  return arrOb.reduce((results, arr) => {
    results[arr[0]] = arr[1]
    return results
  },{})
}
...
const findHighestPriced = (arrObj) => {
  return arrObj.reduce((holder, obj) => {
    if (obj.price > holder.price)
      holder = obj
    return holder
  })
}
...
const reduceArray = (arr, callback, startValue) => {
  let sum = startValue
  arr.forEach((element, index) => {
    sum = callback(sum, element, index)
  })
  return sum
}
/*-----------------------------------------------------------------
...
const flatten = (arr) => {
  return arr.reduce((add, element) => {
    if (Array.isArray(element)) {
     return add.concat(flatten(element))
    } else {
      return add.concat(element)
    }
  },[])
}
/*-----------------------------------------------------------------
...
//// I don't understand why the answer is 2,2,2,5,5 how? So I skopped.
...
const intersection = (arr1, arr2) => {
  ...
  return arr2.filter(el => (arrObj[el] && el === arrObj[el]))
}
/*-----------------------------------------------------------------
...
const getNumForIP = (str) => {
  const regex = /(25[0-6]|2[0-4][0-9]|[01]?[0-9][0-9]?).(25[0-6]|2[0-4][0-9]|[01]?[0-9][0-9]?).(25[0-6]|2[0-4][0-9]|[01]?[0-9][0-9]?).(25[0-6]|2[0-4][0-9]|[01]?[0-9][0-9]?)/
  const ips = str.match(regex)

  for (let i = 1; i < ips.length; i++)
    ips[i] = parseInt(ips[i])
  
  return ips[1] * (256 ** 3) + ips[2] *  (256 ** 2) + ips[3]* (256 ** 1)+ ips[4]
}
/*-----------------------------------------------------------------
...
const toCamelCase = (str) => {
  return str.replace(/[_-]\w/g, (slice) => {
...
const totalTaskTime = (stack, channels ) => {
